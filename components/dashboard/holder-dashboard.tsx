        ...
        <main className="holder-content" id="dashboard-content">
          ...
          {section === "overview" && <>
            <div className="holder-overview-grid">
              ...
              <section className="holder-panel milestone-panel"><div className="panel-heading"><h2>Inside ai swim</h2><Radio size={17} /></div><p className="holder-muted">From first look to favorite</p><ol className="holder-milestones">{phases.map((phase, i) => <li key={phase}><span>{i + 1}</span><div>{phase}</div></li>)}</ol><Link href="/#roadmap" className="holder-text-link">How the network works <ArrowUpRight size={14} /></Link></section>
            </div>
          ...
          {section === "previews" && <><section className="holder-panel holder-empty"><Clapperboard size={36} /><span className="holder-kicker">SHOWS</span><h2>No previews to watch right now.</h2><p>Check back for curated previews and new show announcements.</p><button className="holder-button" onClick={() => navigate("preferences")}>Choose your interests <ArrowUpRight size={16} /></button></section><div className="holder-three-grid slate-steps">{["Find your kind of story", "Watch the preview", "Follow release updates"].map((title, i) => <div className="holder-panel" key={title}><span className="holder-kicker">0{i + 1}</span><h3>{title}</h3></div>)}</div></>}
