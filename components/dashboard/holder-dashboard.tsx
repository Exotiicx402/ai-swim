...
const updates = [
  { title: "Independent voices. Original worlds.", tag: "OUR NETWORK", body: "ai swim brings independent entertainment and its community together. Shows come first: distinctive stories, memorable characters, and craft that holds your attention.", detail: "Explore shows for curated previews and release updates. Check opportunities for paid creative work, and community for participation rounds. Availability is shown in each section." },
...
const phases = ["Find a story that grabs you", "Watch the preview", "Follow release updates", "Come back for the show"];
        ...
        <main className="holder-content" id="dashboard-content">
          ...
          {section === "overview" && <>
            <div className="holder-overview-grid">
              <section className="holder-feature"><div className="broadcast-art" aria-hidden="true"><i /><i /><i /><span>[as]</span></div><div className="feature-content"><span className="holder-badge">INSIDE THE NETWORK</span><h2>Find your next<br />favorite show.</h2><p>Good stories, memorable characters, and craft in every frame. Discover entertainment selected for its quality, not how quickly it was generated.</p><button className="holder-button" onClick={() => navigate("previews")}>Explore shows <ArrowUpRight size={16} /></button></div></section>
              <section className="holder-panel milestone-panel"><div className="panel-heading"><h2>Inside ai swim</h2><Radio size={17} /></div><p className="holder-muted">From first look to favorite</p><ol className="holder-milestones">{phases.map((phase, i) => <li key={phase}><span>{i + 1}</span><div>{phase}</div></li>)}</ol><Link href="/#thesis" className="holder-text-link">How the network works <ArrowUpRight size={14} /></Link></section>
            </div>
            ...
            <div className="holder-three-grid">{[
              { icon: Clapperboard, title: "Discover original worlds", text: "Explore previews selected for story, originality, and craft.", target: "previews" as Section, label: "Explore shows" },
          ...
          {section === "previews" && <><section className="holder-panel holder-empty"><Clapperboard size={36} /><span className="holder-kicker">SHOWS</span><h2>{wallet ? "No previews to watch right now." : "Trailers & pilots are for holders."}</h2><p>{wallet ? "Check back for curated previews and new show announcements." : "Verify your $SWIM holdings to access shows. The guest dashboard does not include playback."}</p>{wallet ? <button className="holder-button" onClick={() => navigate("preferences")}>Choose your interests <ArrowUpRight size={16} /></button> : <Link href="/dashboard" className="holder-button">Verify holder access <LockKeyhole size={16} /></Link>}</section><div className="holder-three-grid slate-steps">{["Find your kind of story", "Watch the preview", "Follow release updates"].map((title, i) => <div className="holder-panel" key={title}><span className="holder-kicker">0{i + 1}</span><h3>{title}</h3></div>)}</div></>}
